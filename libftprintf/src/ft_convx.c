/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_convx.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cdarci <cdarci@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/30 19:51:19 by cdarci            #+#    #+#             */
/*   Updated: 2019/11/02 17:43:18 by cdarci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

static int	ft_specx(char *num, t_params *params)
{
	int	len;

	len = 0;
	if (num[0] == '0' && (num[1] == 'x' || num[1] == 'X'))
	{
		if (num[0] == '0' && num[1] == 'x')
			len += ft_putstrf("0x", 2);
		else if (num[0] == '0' && num[1] == 'X')
			len += ft_putstrf("0X", 2);
		len += ft_putcharntime('0', params->len - (int)ft_strlenf(num));
		len += ft_putstrf(num + 2, ft_strlenf(num) - 2);
	}
	else
	{
		len += ft_putcharntime('0', params->len - (int)ft_strlenf(num));
		len += ft_putstrf(num, ft_strlenf(num));
	}
	return (len);
}

static int	ft_putnx(char *num, t_params *params)
{
	int	len;

	len = 0;
	if (params->left)
		if (params->len > ft_strlenf(num))
		{
			len += ft_putstrf(num, ft_strlenf(num));
			len += ft_putcharntime(' ', params->len - (int)ft_strlenf(num));
		}
		else
			len += ft_putstrf(num, ft_strlenf(num));
	else
	{
		if (params->len > ft_strlenf(num))
			if (params->zero && params->acc == -1)
				len += ft_specx(num, params);
			else
			{
				len += ft_putcharntime(' ', params->len - (int)ft_strlenf(num));
				len += ft_putstrf(num, ft_strlenf(num));
			}
		else
			len += ft_putstrf(num, ft_strlenf(num));
	}
	return (len);
}

static char	*ft_set(char *sign, char *n, t_params *params)
{
	char *res;

	if (params->acc > (int)ft_strlenf(n))
	{
		res = ft_strjoindelboth(ft_strbzero(params->acc - ft_strlenf(n)), n);
		if (sign[0] != '\0')
			res = ft_strjoindelboth(sign, res);
	}
	else
	{
		if (n[0] == '0')
		{
			ft_strdelf(&n);
			if (sign)
				ft_strdelf(&sign);
			res = ft_strnewf(0);
		}
		else
			res = ft_strjoindelboth(sign, n);
	}
	return (res);
}

static char	*ft_setcurrent(char *num, t_params *params)
{
	char *res;
	char *sign;
	char *n;

	sign = ft_strnewf(2);
	if (num[0] == '0' && (num[1] == 'x' || num[1] == 'X'))
	{
		sign = ft_strncpyf(sign, num, 2);
		n = ft_strdupf(num + 2);
	}
	else
		n = ft_strdupf(num);
	res = ft_set(sign, n, params);
	return (res);
}

int			ft_convx(uintmax_t num, t_params *params, char letter)
{
	int		len;
	char	*tmp;
	char	*new_num;

	len = 0;
	new_num = ft_itoa_base(num, 16);
	if (params->altform && num != 0)
	{
		tmp = ft_strjoinf("0x", new_num);
		ft_strdelf(&new_num);
		new_num = tmp;
	}
	if (letter == 'X')
	{
		tmp = ft_strmapf(new_num, &ft_toupperf);
		ft_strdelf(&new_num);
		new_num = tmp;
	}
	if (params->acc >= 0)
		new_num = ft_setcurrent(new_num, params);
	len += ft_putnx(new_num, params);
	ft_strdelf(&new_num);
	free(params);
	return (len);
}
