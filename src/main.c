...
static void		ft_printall_dirs(t_allargs *args, t_lsflags *flags)
{
	t_list	*point;
	size_t	count;
	size_t	size;
	...
	while (point)
	{
		if (count == 0 && ((!args->singles && size != 1) || \
(args->errors && !args->singles)))
			ft_printf("%s:\n", point->content);
		else if (args->errors || args->singles || \
(count < size && size != 1))
			ft_printf("\n%s:\n", point->content);
...
static void		ft_printall_args(t_allargs *args, t_lsflags *flags)
{
	ft_lstiter(args->errors, lstprint_errors);
	flags->single = 1;
	ft_printfiles(args->singles);
	flags->single = 0;
	ft_printall_dirs(args, flags);
